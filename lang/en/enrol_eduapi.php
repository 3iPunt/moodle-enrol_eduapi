...
$string['settings_connection_root_url_desc'] = 'The provider\'s Edu-API v1p0 API root, including any vendor-specific path. Use the \'servers\' URL declared in the provider\'s OpenAPI discovery document (for example https://example.org/ims/eduapi/base/v1p0), not the hostname of the discovery document itself. Used exactly as entered: no path is appended automatically.';
...
$string['settings_datasync_academic_session_desc'] = 'The single academic session to synchronise. Populated by "Test connection" above. Selecting a session also includes its child sessions (for example, selecting a school year also includes its semesters).';
...
$string['settings_multilang'] = 'Multilang support';
$string['settings_multilang_desc'] = 'When enabled, course fullnames, group names and the course summary (see the "Sync description as summary" setting) are built from every available language, wrapping each Edu-API recordLanguage in a `<span lang="..." class="multilang">...</span>` tag. Rendering that markup as separate languages requires the site\'s "Multi-language content" filter (filter/multilang) to be enabled; if it is not, the raw spans are shown as-is. When disabled (the default), a single value is picked instead: the one matching the site\'s default language, else English, else the first available one.';
...
$string['settings_sync_description'] = 'Sync description as summary';
$string['settings_sync_description_desc'] = 'Copy the offering\'s description into the Moodle course summary at every sync, preserving the provider\'s line breaks. When disabled, the course summary is never written by the sync. An offering with no description never blanks an existing course summary.';
$string['settings_sync_groups'] = 'Sync groups';
$string['settings_sync_groups_desc'] = 'Create a Moodle group for every offering at the level not chosen as the course (e.g. ComponentOfferings, when CourseOffering is the course level). Group membership is kept in sync from the component offering\'s own enrolments: users enrolled at the component level are added to the group, and users withdrawn, cancelled or deleted are removed. Only applies to a user who also holds a CourseOffering-level enrolment; a component-only enrolment is skipped.';
...
$string['settings_user_field_department_source'] = 'Department source';
$string['settings_user_field_institution_source'] = 'Institution source';
$string['settings_user_field_source_desc'] = 'Optional. The Person attribute to copy into the Moodle user\'s "{$a}" field, using the same grammar as the Edu-API source attribute setting above: `primaryEmail` and `sourcedId` are also accepted, `extensions.<key>` reads the top-level `extensions.<key>` value, and `otherIdentifiers.<identifierType>` reads the identifier of the first `otherIdentifiers` entry of that type. Leave empty to disable. An absent or empty attribute leaves the field untouched: it is never blanked by the sync. Whether the field is also kept up to date on existing users at every sync is controlled by "Update existing users\' fields" below.';
$string['settings_user_field_update_existing'] = 'Update existing users\' fields';
$string['settings_user_field_update_existing_desc'] = 'When enabled (the default), the department/institution sources configured above are also applied to already-existing users at every sync: any field whose resolved value differs from what is stored is updated (and only that field), triggering the standard user_updated event like any other Moodle profile update. When disabled, those sources only seed the field when a new Moodle user is created; existing users are never modified.';
...
$string['settings_user_match_source_desc'] = 'The Person attribute to match against the Moodle field above: `primaryEmail`, `sourcedId`, or an `otherIdentifiers` type.';
