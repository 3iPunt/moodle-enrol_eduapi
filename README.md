...
| **Education offering** | Which offering level becomes a course (`CourseOffering` or `ComponentOffering`; the level not chosen becomes groups) and the shortname attribute. Group membership is kept in sync from the `ComponentOffering`'s own enrolments, but only for a Person who also holds a `CourseOffering`-level enrolment; a component-only enrolment is skipped. `sync_description` (on by default) maps the offering's description onto the course summary, without ever blanking an existing one. `multilang` (off by default) builds course fullnames, group names and the summary from every available language instead of a single one; rendering the result as separate languages needs the site's "Multi-language content" filter (`filter/multilang`) enabled. |
| **User matching** | Moodle field vs Edu-API attribute, with persistent mapping and an option to create unmatched users. Both the match attribute and the optional `department`/`institution` mappings share the same grammar: `primaryEmail`, `sourcedId`, a Person `extensions.<key>` or an `otherIdentifiers.<identifierType>` attribute (an absent or empty attribute leaves `department`/`institution` untouched); a separate setting, enabled by default, controls whether existing users are also kept up to date at every sync. |
